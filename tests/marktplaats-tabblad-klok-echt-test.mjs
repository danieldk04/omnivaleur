/**
 * De échte proef: loopt de klok van een MARKTPLAATS-werktabblad ook als het niet
 * in beeld staat?
 *
 * Daniel, 18-09-2026: "op marktplaats publiceerde hij weer niet zonder mijn
 * invloed — naar het tabblad gaan en alleen op publish drukken." Zijn
 * 2dehands-opdracht stuurde op dat moment deze voortgang door, uit zijn eigen
 * browser, met de debugger er gewoon aan:
 *
 *     klokstand: 0.1/s +23546ms hidden
 *
 * Eén tik per tien seconden in plaats van tien per seconde. In de code stond
 * sinds 15-09 dat Marktplaats hier geen last van had "want daar hangt de
 * debugger al aan het tabblad". Die aanname is hiermee weerlegd: aanhechten
 * haalt de rem er niet af, alleen Emulation.setFocusEmulationEnabled doet dat,
 * en die stond alleen aan voor het Vinted-formulier.
 *
 * Deze proef laadt de échte extensie in een echte Chrome, laat haar langs de
 * gewone weg een werk-tabblad openen op het Marktplaats-plaatsformulier, en
 * meet daarna in dat tabblad hoe vaak de klok van de pagina werkelijk tikt.
 *
 * Er wordt niets gepubliceerd: zonder Marktplaats-sessie komt het tabblad op de
 * inlogpagina uit, en dat is voor deze meting genoeg — het gaat om de klok van
 * het tabblad, niet om het formulier.
 *
 * Draaien: node tests/marktplaats-tabblad-klok-echt-test.mjs [pad naar extensie]
 * Voor-en-na-proef met de versie van vóór de reparatie:
 *   git archive <commit>:extension | tar -x -C /tmp/oud
 *   node tests/marktplaats-tabblad-klok-echt-test.mjs /tmp/oud
 * Zonder de reparatie hoort de klok op 10 tikken of minder te blijven steken.
 */
...
const FORMULIER = "https://www.marktplaats.nl/plaats/1776/649?bucketId=169&title=";
...
try {
  ...
  // Precies wat een echte Marktplaats-opdracht doet: een achtergrondtabblad in
  // het venster waar de verkoper toch al werkt. Geen eigen venster — die weg is
  // alleen voor het Vinted-formulier.
  ...
  // maakWerkTabblad hecht de debugger aan vóór het navigeren en zet daar de klok
  // aan; dat is precies wat hier gemeten wordt.
  ...
  // Wachten tot Marktplaats geladen is (zonder sessie: de inlogpagina).
  for (let i = 0; i < 40; i++) {
  ...
  const pagina = targets.result.targetInfos.find((t) => t.type === "page" && /marktplaats\./.test(t.url));
  check("het tabblad staat op Marktplaats", !!pagina, targets.result.targetInfos.map(t => t.url).join(" | ").slice(0, 200));
  if (!pagina) throw new Error("geen Marktplaats-tabblad");
  ...
  // Een echte Marktplaats-pagina doet zelf ook werk, dus de 100 van een lege
  // pagina wordt hier niet gehaald. Zonder de reparatie zijn het er
  ...
  // het teken dat er zonder focus-emulatie niets aan de rem verandert.
