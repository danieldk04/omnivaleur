...
async def reconcile_alle_shopify_winkels() -> dict:
    """Elke gekoppelde Shopify-winkel een reconciliatieronde geven.

    Draait dagelijks (zie scheduler.py) zodat een product dat buiten Omnivaleur
    om aan de catalogus wordt toegevoegd — bulkimport, handmatig in het
    winkelbeheer — vanzelf wordt herkend, zonder dat de verkoper iets hoeft te
    doen of zelfs maar hoeft in te loggen.
    """
    db = get_db()
    ...
    for rij in rijen:
        try:
            ...
            logger.warning("shopify-reconciliatie: ronde mislukt voor %s: %s", rij["user_id"], e)
    if totaal_gekoppeld:
        logger.info("shopify-reconciliatie: %d artikel(en) gekoppeld over %d winkel(s)",
                    totaal_gekoppeld, len(rijen))
